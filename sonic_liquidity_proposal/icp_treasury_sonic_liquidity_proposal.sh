...
TITLE="Transfer 2390 ICP to ICP/CAT Pool on Sonic Dex"
...
quill sns  \
   ...
   --pem-file "${PEM_FILE}"  \
   make-proposal --proposal "(record { title=\"${TITLE}\"; url=\"${URL}\"; summary=\"${SUMMARY}\"; action=opt variant {TransferSnsTreasuryFunds = record {from_treasury=1:int32; amount_e8s=${AMOUNT_E8s}:nat64; to_principal=opt principal \"${TO_PRINCIPAL}\";memo=null; to_subaccount=opt ${SUBACCOUNT};}}})" "${DEVELOPER_NEURON_ID}" > msg.json
