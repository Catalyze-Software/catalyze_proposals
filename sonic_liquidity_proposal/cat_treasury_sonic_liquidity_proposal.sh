...
TITLE="Transfer 500000 CAT to ICP/CAT Pool on Sonic Dex"
URL="https://catalyze.one"
SUMMARY="
This is the first of 2 proposals which add liquidity to the [ICP/CAT](https://data.sonic.ooo/pools/ryjl3-tyaaa-aaaaa-aaaba-cai:uf2wh-taaaa-aaaaq-aabna-cai) pool on SONIC DEX\n\n
...
quill sns  \
   ...
   --pem-file "${PEM_FILE}"  \
   make-proposal --proposal "(record { title=\"${TITLE}\"; url=\"${URL}\"; summary=\"${SUMMARY}\"; action=opt variant {TransferSnsTreasuryFunds = record {from_treasury=2:int32; amount_e8s=${AMOUNT_E8s}:nat64; to_principal=opt principal \"${TO_PRINCIPAL}\";memo=null; to_subaccount=opt record {subaccount= vec {${SUBACCOUNT}};}}}})" "${DEVELOPER_NEURON_ID}" > msg.json
